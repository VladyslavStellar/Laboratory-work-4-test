...
CREATE TABLE locations (
    ...
    CONSTRAINT chk_coordinates_format CHECK (coordinates ~ '^-?\d{1,3}\.\d+, -?\d{1,3}\.\d+$'),
...
CREATE TABLE air_quality_logs (
    ...
    CONSTRAINT fk_aqi_location FOREIGN KEY (location_id) REFERENCES locations(location_id) ON DELETE CASCADE,
...
CREATE TABLE poetry_collections (
    ...
    CONSTRAINT chk_year CHECK (publication_year <= EXTRACT(YEAR FROM CURRENT_DATE))
);
...
CREATE TABLE poems (
    ...
    CONSTRAINT fk_poem_author FOREIGN KEY (author_id) REFERENCES authors(author_id) ON DELETE CASCADE,
    CONSTRAINT fk_poem_collection FOREIGN KEY (collection_id) REFERENCES poetry_collections(collection_id) ON DELETE SET NULL,
...
CREATE TABLE user_saved_poems (
    ...
    CONSTRAINT fk_saved_poem FOREIGN KEY (poem_id) REFERENCES poems(poem_id) ON DELETE CASCADE,
